...
import { agenciasSchema, AgenciasSchema } from "@/schemas/agencias/agencias.schemas";
import { zodResolver } from "@hookform/resolvers/zod";
import { MutationAgencias, MutationUpdateAgencia } from "@/hooks/agencias/useMutationAgencias";
import { DialogFooter } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { AgenciasModelTypes } from "@/interfaces/agencias.interfaces";
import { Badge } from "@/components/ui/badge";
export const FormAddAgency = ({ closeModal, items }: { closeModal: () => void; items?: AgenciasModelTypes }) => {
  const { ref, inView } = useInView();
  const isEdit = Boolean(items);

  const form = useForm<AgenciasSchema>({
    resolver: zodResolver(agenciasSchema),
    defaultValues: {
      code: items?.code || "",
      name: items?.name || "",
  ...
  const mutationAgencias = MutationAgencias(closeModal);
  const mutationUpdateAgencia = MutationUpdateAgencia(closeModal);
  const onSubmit = async (data: AgenciasSchema) => {
    if (isEdit) {
      await mutationUpdateAgencia.mutateAsync({ id: items!.id, data });
    } else {
      await mutationAgencias.mutateAsync(data);
    }
  };
  useEffect(() => {
  ...
  return (
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} id="form-add-agency">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="w-full">
                  <FormLabel>Código de la agencia</FormLabel>
                  <FormControl>
          ...
          <div className="mt-4 space-y-2 rounded-lg">
            <div>
              <Label>Jefe de la agencia</Label>
              <div className="flex items-center justify-between gap-2">
                <Input
                  className="max-w-3xs"
                  startContent={<Search />}
                  type="text"
                  placeholder="Buscar jefe de la agencia"
                  value={searchNombre}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex w-full flex-col py-4">
                <FormLabel htmlFor="state">Estado de la agencia</FormLabel>
      ...
      <DialogFooter>
        ...
        <Button type="submit" variant="custom2" form="form-add-agency" disabled={mutationAgencias.isPending || mutationUpdateAgencia.isPending}>
          {mutationAgencias.isPending || mutationUpdateAgencia.isPending ? <Loader2 className="animate-spin" /> : <Save />}
          Guardar
        </Button>
      </DialogFooter>
    </>
