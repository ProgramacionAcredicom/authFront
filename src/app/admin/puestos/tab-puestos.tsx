...
export default function TabPuestos() {
  ...
  const canListPuestos = hasAccess(userQuery.data, OAUTH_PERMISSIONS.LIST_POSITIONS);
  const canCreatePuesto = hasAccess(userQuery.data, OAUTH_PERMISSIONS.CREATE_POSITION);
  const canUpdatePuesto = hasAccess(userQuery.data, OAUTH_PERMISSIONS.UPDATE_POSITION);
  const canDeactivatePuesto = hasAccess(userQuery.data, OAUTH_PERMISSIONS.DEACTIVATE_POSITION);
  const { queryPuestos } = useQueryPuestos(undefined, { enabled: canListPuestos });
    ...
    <>
      <div className="flex flex-col gap-6 px-4 py-4">
        <header className="flex items-center justify-between">
          ...
          {canCreatePuesto ? <ModalAgregarPuesto /> : null}
        </header>
        ...
        {userQuery.isLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center px-4 py-16">
              ...
              <TypographyH3 text="Sin permisos para listar puestos" className="mb-2 text-center" />
              <TypographyMuted
                text="Tu usuario puede ingresar al módulo, pero no tiene permiso para consultar el listado de puestos."
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center px-4 py-16">
              ...
              />
              {!searchTerm && canCreatePuesto ? <ModalAgregarPuesto /> : null}
            </CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {data.map((puesto: PuestoListItem) => (
              <CardPuesto
                key={puesto.id}
                puesto={puesto}
                onEdit={setEditingPuestoId}
                canEdit={canUpdatePuesto}
                canDeactivate={canDeactivatePuesto}
              />
            ))}
          </div>
        )}
